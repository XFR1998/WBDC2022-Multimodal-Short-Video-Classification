...
def create_dataloaders(args):
    if args.use_all_pretrain_data == True:
        print('使用所有数据预训练')
        unlabel = MultiModalDataset(args, '../../data/annotations/unlabeled.json', '../../data/zip_feats/unlabeled.zip')
        label = MultiModalDataset(args, '../../data/annotations/labeled.json', '../../data/zip_feats/labeled.zip')
        test = MultiModalDataset(args, '../../data/annotations/test_a.json', '../../data/zip_feats/test_a.zip')
...
class MultiModalDataset(Dataset):
    ...
    def __init__(self,
        ...
        # 文本预处理：预处理title,asr,ocr

        # print('---------------------预处理title,asr,ocr-----------------------')
        ...
        #
        # for i in tqdm(range(len(self.anns))):
        #     temp_ocr = CharTable.convert(self.anns[i]['ocr']) # 繁体 -> 简体，全角 -> 半角、大写 -> 小写
        #     clean_ocr = ht.clean_text(temp_ocr, emoji=False) # 去除链接、@xxx、email
        #     self.anns[i]['ocr'] = clean_ocr
        #
        #
        #     temp_title = CharTable.convert(self.anns[i]['title'])
        #     clean_title = ht.clean_text(temp_title, emoji=False)  # 去除链接、@xxx、email
        #     self.anns[i]['title'] = clean_title
        #
        #     temp_asr = CharTable.convert(self.anns[i]['asr'])
        #     clean_asr = ht.clean_text(temp_asr, emoji=False)  # 去除链接、@xxx、email
        #     self.anns[i]['asr'] = clean_asr
        # print('---------------------预处理完毕-----------------------')
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        cat_input, cat_mask = self.tokenize_text(cat_text, max_len=468)
