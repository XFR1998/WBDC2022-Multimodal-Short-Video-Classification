...
from data.pre_data_kfold import MultiModalDataset
...
def inference():
    args = parse_args()
    # 1. load data
    print(args.test_annotation, args.test_zip_feats)
    ...
    for kfold_idx in tqdm(range(1, 6)):
        kfold_logits = []
        kfold_model_path = args.saved_kfold_model_path + f'/{kfold_idx}fold_model/'
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader):
                logits, pred_label_id = model(batch, inference=True, return_logits=True)
    ...
    np.save(args.saved_kfold_logits + "kfold_logits_reberta_mlm_mfm_itm_20ep_15_testb.npy", save_logits)
    ...
    test_output_csv = args.test_output_csv.replace('result.csv', 'result_logits_avg_testb.csv')
    with open(test_output_csv, 'w') as f:
        for pred_label_id, ann in zip(predictions, dataset.anns):
            video_id = ann['id']
            category_id = lv2id_to_category_id(pred_label_id)
